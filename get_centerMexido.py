...
def change_perspective(img):
  ...
  print(dst)
  src = np.float32([[0, 320], [240, 320], [215, 50], [40, 50]])
  print(src)
...
def lr_curvature(binary_warped):
 ...
 #out_img = np.dstack((binary_warped, binary_warped, binary_warped))*255
  ...
  dist_from_center =  actual_center - (320/2)
...
def process_image(img):
  ...
  #trapezoid = np.array([[570, 420], [160, 320], [1200, 320], [700, 420]], np.int32);
  ...
  linha = np.array([[actual_center, 0], [actual_center, 320]], np.int32); #Cria um array de dois pontos onde a linha para auxilio visual será criado
  masked_image = region_of_interest(img, [linha])#aqui era undist_img / criar linha na imagem
  BlendImage = cv2.addWeighted(img, 1, masked_image, 0.3, 0) 
  plt.imshow(BlendImage, cmap='gray') #Fundir as imagens
  plt.title('masked_image')
  plt.show()
...
if __name__ == '__main__':
  ...
  while(True):
   try: 
     ...
     #image2 = mpimg.imread('white.png')
     #image3 = cv2.addWeighted(image1, 1,image2, 0.3, 0)
     #plt.imshow(image2, cmap='gray')
     #plt.show()
     colored_image = process_image(image1)
